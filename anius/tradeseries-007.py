...
def _generate_query_string():
    ...
    query_string = 'select * from 5min2 where symbol="@NQ" and date="2013-10-22";'
    '''
    n = dt.datetime(2013, 10, 01)
    s = dt.datetime(2012, 11, 30)
    for i in range((n -s).days + 1):
        yield (s+dt.dimedelta(days = i)).date()
...
class TradeSeries(object):
    ...
    def capture_traderun(self):
        ...
        while(True):                                        # Need the while loop around the whole routine, because this is a generator/yield
            ...
            logger.debug("TradeSeries.capture_traderun(): yielding df3")
            yield df3                                  # The new timeseries will be normalized to +/- zero vis-a-vis price and time

            if index_end < len(self.df):
                logger.debug("TradeSeries.capture_traderun(): Not yet at end of tradeseries, continuing...")
                index_start += index_end + 1
                continue
            else:
                logger.debug("TradeSeries.capture_traderun(): At end of tradeseries, yielding None")
                yield None
