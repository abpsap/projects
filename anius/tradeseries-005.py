    ...
    def capture_traderun(self,index_start,index_end):
        ...
        def _traderun_is_not_contiguous():
            ...
            if df2.index.item(len(df2)-1) - df2.index.item(0) <> len(df2)-1:
                logger.info("Skipping non-contiguous traderun", df2.index.item(0), df2.index.item(len(df2)-1), len(df2))
...
#    while(True):                                        # Need the while loop around the whole routine, because this is a generator/yield
        ...
        return(df3)                                # The new timeseries will be normalized to +/- zero vis-a-vis price and time
...
if __name__ == '__main__':    
    ...
    mytr = myts.capture_traderun(100,200)   
